...
**Remaining work**: no live `window.rs` display yet — verification so far is via a one-shot PPM/PNG dump after N frames, not an actual rendered window a user could interact with. Wiring the existing (currently-unused) `window.rs` winit/softbuffer scaffolding to the network loop's surface updates — likely via a background thread + channel, since winit's event loop must own the main thread — is the next real step toward a usable client. The previously-researched RemoteFX Progressive/RLGR1 decoder (`phase2_gfx_pipeline.md` memory) is still unexercised by this host; may be genuinely dead code for this specific server, or may appear for full-frame/video-like content not yet triggered in testing.

Debug `eprintln!`s from earlier blockers are still in place in `main.rs`'s `ChannelRouter` and `dvc.rs`'s `handle_message`; the ZGFX/ClearCodec-specific trace instrumentation added and used during this session's debugging has been removed now that the bugs are fixed.

See memory (`phase2_gfx_pipeline.md`, `zgfx_compression.md`, and a new `clearcodec_and_surfaces.md`) for the full byte-level reference, and `phase1_mcs_capability_pdus.md` for the Phase 1 equivalent.
